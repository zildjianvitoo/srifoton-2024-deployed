...
function QNA({title, id, children}: QNAProps) {
  ...
  return (
    <section id={id} className="hidden lg:flex flex-col gap-[35px] relative">
      <div className="w-[78%] mx-auto">
        <SectionHeader text={title} />
      </div>

      <div className="flex justify-between gap-24">
        <div className="flex flex-col relative max-w-[686px] gap-[30px]">
          ...
          {Children.map(children as ReactElement, (content, index) => {
            ...
            return(
              <div className={`flex flex-col ${isHighlighted ? "block" : "hidden"}`}>
                <div className="max-w-[666px] bg-[#D9D9D9] relative xl:text-xl 2xl:text-2xl p-6 xl:p-11 rounded-3xl">
                  <p>
                ...
                <Image src="/img/gallery1.png" alt="reviewer" width={534} height={467} className="aspect-[534/467] flex self-end mx-auto min-w-[67.4%] object-contain"></Image>
              </div>
            )
          })}
        </div>
...
function SpeechBubble({id, question, highlighted = false} : SpeechBubbleProps) {
  ...
  var [ contentState, svgState ] = highlighted ?
    ["max-w-[686px] min-h-[80px] xl:min-h-[123px] bg-[#D9D9D9] xl:text-xl 2xl:text-2xl",
      {width:"48", height:"22", fill:"#d9d9d9"}] :
    ["w-[86.15%] min-h-[70px] xl:min-h-[100px] bg-[#b7b38c] text-sm xl:text-lg 2xl:text-xl",
