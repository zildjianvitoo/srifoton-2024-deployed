...
export const getWorkshopsByUserId = async (
  userId: string,
): Promise<Workshop[]> => {
  ...
  try {
    ...
    await fetch("http://139.99.89.108:3005/event-srifoton", {
      ...
      body: JSON.stringify({
        name: workshop.name,
        eventName: "Workshop",
        paymentType: workshop.payment_method,
      }),
    });
...
export const generateWorkshopTicketNumberAndVerifyById = async (
  id: string,
): Promise<string> => {
  try {
...
export const updateWorkshop = async (
  id: string,
  updatedDetails: Partial<Workshop>,
): Promise<void> => {
  try {
    const docRef = doc(db, "workshops", id);
