...
function verifyCookie(value: string | undefined, currentKey: string, previousKey: string) {
  if (!value) return "";
  const unsignedValue = cookieSignature.unsign(value, currentKey) || cookieSignature.unsign(value, previousKey);
...
export default function middleware(request: NextRequest) {
  const userSession = verifyCookie(request.cookies.get(USER_SESSION_NAME)?.value, AUTH_COOKIE_SIGNATURE_KEY_CURRENT, AUTH_COOKIE_SIGNATURE_KEY_PREVIOUS);
  const adminSession = verifyCookie(request.cookies.get(ADMIN_SESSION_NAME)?.value, ADMIN_COOKIE_SIGNATURE_KEY_CURRENT, ADMIN_COOKIE_SIGNATURE_KEY_PREVIOUS);
  const userToken = verifyCookie(request.cookies.get(AUTH_COOKIE_NAME)?.value, AUTH_COOKIE_SIGNATURE_KEY_CURRENT, AUTH_COOKIE_SIGNATURE_KEY_PREVIOUS);
  const adminToken = verifyCookie(request.cookies.get(ADMIN_COOKIE_NAME)?.value, ADMIN_COOKIE_SIGNATURE_KEY_CURRENT, ADMIN_COOKIE_SIGNATURE_KEY_PREVIOUS);

  console.log("Masuk middleware");

  // Redirect to login if no session and not on a public route
  if (!userSession && !adminSession && !publicRoutes.includes(request.nextUrl.pathname)) {
  ...
  if (request.nextUrl.pathname.startsWith("/account-data") || request.nextUrl.pathname.startsWith("/personal-data") || request.nextUrl.pathname.startsWith("/activities") && !userToken) {
    const absoluteURL = new URL("/login", request.nextUrl.origin);
    if (isValidRedirectionPath(absoluteURL.pathname)) {
      return NextResponse.redirect(absoluteURL.toString());
    }
